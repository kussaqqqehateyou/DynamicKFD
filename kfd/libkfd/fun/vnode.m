...
uint64_t getVnodeVar(void) {
    ...
    //5
    puts("finding vnode var"); sleep(1);
    ...
    printf("got vnode for path\n");
    
    uint64_t parent_vnode = vnode;
    for(int i = 0; i < 5; i++) {
        
        parent_vnode = unsign_kaddr(kread64(parent_vnode + off_vnode_v_parent));
        
        if (parent_vnode == 0) {
            printf("FUCK"); usleep(5000);
            do_kclose(_kfd);
            exit(0);
        }
...
uint64_t findChildVnodeByVnode(uint64_t vnode, const char* childname) {
    uint64_t vp_nameptr = kread64(vnode + off_vnode_v_name);
    uint64_t vp_name = kread64(vp_nameptr);
